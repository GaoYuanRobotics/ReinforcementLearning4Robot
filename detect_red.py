...
def detect_ball():
    ...
    Image = cv2.imread('/home/lu/pycode_lxc/image/rgb.png')
    Gray = cv2.cvtColor(Image, cv2.COLOR_BGR2GRAY)
    rows, cols, channels = Image.shape
    diff =100 #detect red: 100, blue 30~50.
    min_row = rows
    max_row = 0
    min_col = cols
    max_col = 0
    ...
    for row in range(rows):
        for col in range(cols):
            if Image[row, col, 2] - Image[row, col, 0] > diff:
                if Image[row, col, 2] - Image[row, col, 1] > diff:
                    Gray[row, col] = 255
                    if (min_row > row):
                        min_row = row
                    if (max_row < row): 
